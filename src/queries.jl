...
"""
	query_to_blog(q::Query)::Vector{String}
...
transformed into BLOG syntax).
"""
function query_to_blog(q::Query)::Vector{String}
	result = []
	for (evidence, assignment) in q.evidence
		push!(result, string("obs", " ", evidence, " = ", assignment, ";"))
	end
	push!(result, string("query", " ", q.query_var, ";"))
	return result
end